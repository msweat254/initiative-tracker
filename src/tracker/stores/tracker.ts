    ...
    const setState = (state: boolean) => {
        ...
        if (state) {
            if (!_logger?.logging) {
                ...
                void _logger?.log(`Combat re-started`);
            }
        } else {
            void _logger?.log("Combat stopped");
        }
    ...
    return {
                ...
                if (current == -1) {
                    ...
                    if (next) {
                        ...
                        if (nextIndex < current) {
                            ...
                            void _logger?.log("###", `Round ${round}`);
                        }
                        void _logger?.log("#####", `${next.getName()}'s turn`);
                        next.active = true;
                    }
                }
                return creatures;
            ...
            updateAndSave((creatures) => {
                ...
                if (current == -1) {
                    ...
                    if (prev) {
                        ...
                        void _logger?.log("#####", `${prev.getName()}'s turn`);
                        prev.active = true;
                    }
                }
                return creatures;
        ...
        new: (plugin: InitiativeTracker, state?: InitiativeViewState) =>
            updateAndSave((creatures) => {
                ...
                if (state?.logFile) {
                    void _logger?.new(state.logFile).then(() => {
                        $logFile.set(_logger.getFile());
                    });
                }
